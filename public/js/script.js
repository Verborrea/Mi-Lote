...
let distritos = {
  ...
  'b' : "Jacobo Hunter",
  'c' : "Sabandia",
  'd' : "Jose Luis Bustamante y Rivero",
  'e' : "Tiabaya",
  'f' : "Sachaca",
  'g' : "Characato",
  'h' : "Paucarpata",
...
function GetMap(){
  ...
  map = new Microsoft.Maps.Map("#map", {
    ...
    center: new Microsoft.Maps.Location(-16.398835, -71.536964),
    zoom: 12,
  });
  ...
  Microsoft.Maps.Events.addHandler(map, 'viewchangeend', function () {
    if(map.getZoom() > 15){
      ...
      let nlim = [Math.floor((vb[0]-cuad[0][1])/pasoy),
                  Math.floor((vb[3]-cuad[0][0])/pasox),
                  Math.floor((vb[2]-cuad[0][1])/pasoy),
                  Math.floor((vb[1]-cuad[0][0])/pasox)];
      let check = false;
      for(let i = 0; i < 4; i++) {
        if(lim[i] !== nlim[i]){
          check = true;
        ...
        for(let i = 0; i < lenc; i++){
          if (lLayers[cargar[i]] === undefined){
            ...
            Microsoft.Maps.Events.addHandler(lLayers[cargar[i]], 'click', (e) => 
            {
              ...
              document.getElementById('distrito-info').innerHTML = distritos[lote_sel.metadata.d];
...
function draw(data, scolor, fcolor, tcolor) {
  ctx2.lineWidth = 3;
  ctx2.beginPath();
  ctx2.moveTo(data.x_px[0], data.y_px[0]);  //aumentar 10 a todo
  for(let i = 1; i < data.len; i++){
    ctx2.lineTo(data.x_px[i], data.y_px[i]);
  }
  ctx2.closePath();
  ...
  for(let i = 0; i+1 < data.len; i++){
    ctx2.translate(data.lx_px[i],data.ly_px[i]);
    ctx2.rotate(data.angulos[i]);
    ctx2.fillText(redondear(data.distances[i].toString(), 0) + 'm', 0, 0);
    ctx2.rotate(-data.angulos[i]);
    ctx2.translate(-data.lx_px[i],-data.ly_px[i]);
  }
}

function posicionCentroTexto(a_x, a_y, b_x, b_y, centroide){
  let ortogonal_x = b_y - a_y;
  let ortogonal_y = a_x - b_x;
  
  let norma = Math.sqrt(ortogonal_x ** 2 + ortogonal_y ** 2);
  
  let ortonormal_20_x = 20 * ortogonal_x/norma;
  let ortonormal_20_y = 20 * ortogonal_y/norma;
  
  let punto_medio_x = (a_x + b_x)/2;
  let punto_medio_y = (a_y + b_y)/2;
  
  let p_x = punto_medio_x + ortonormal_20_x;
  let p_y = punto_medio_y + ortonormal_20_y;

  return [p_x,p_y];
}

function calcAngulo(opposite, adjacent) {
  let angle = Math.atan(opposite / adjacent) + Math.PI/2;
  if(angle < 0){
      angle += 2 * Math.PI;
  }
...
function transform_rings(rings){
  let len = rings.length;
  let x_array =[]; let y_array = [];
  let centroide = Microsoft.Maps.SpatialMath.Geometry.centroid(
    new Microsoft.Maps.Polygon(rings)
  );
  if (centroide == null){
    let x_px = [];
    let y_px = [];
    ...
    let distances = [];
    len = 0;
    return {x_px, y_px, lx_px, ly_px, angulos, distances, len};
  }
  for(let i = 0; i <len; i++){
    x_array.push(rings[i].longitude);
    y_array.push(rings[i].latitude);
  }
  centroide = [centroide.longitude, centroide.latitude];
  let distances = [];
  ...
  let y_px = [];
  for(let i = 0; i < len; i++){
    x_px[i] = x_array[i] * fact_esc + can2_margin;
    y_px[i] = can2.width - can2_margin - y_array[i] * fact_esc;
  }
  centroide[0] *= fact_esc + can2_margin;
  centroide[1] = can2.width - can2_margin - centroide[1] * fact_esc;;
  let lx_px = [];
  let ly_px = [];
  let label_px = [];
  for(let i = 0; (i+1) < len; i++){
    label_px[i] = posicionCentroTexto(x_px[i],y_px[i],x_px[i+1],y_px[i+1], centroide);
    lx_px[i] = label_px[i][0];
    ly_px[i] = label_px[i][1];
  }
  // ==================== saving angles for rotate text ====================
  let angulos = [];
  for(let i = 0; (i+1) < len; i++){
    let v_x = x_px[i+1]-x_px[i];
    let v_y = y_px[i+1]-y_px[i];
    angulos[i] = calcAngulo(v_x, -v_y);
  }
  return {x_px, y_px, lx_px, ly_px, angulos, distances, len}
}
...
function draw_lote_2d(){
  ...
  let centroide = Microsoft.Maps.SpatialMath.Geometry.centroid(lote_sel);
  ...
  // ==================== to_canvas units ====================
  for(let i = 0; i < len; i++){
    lote_x_px[i] = x_array[i] * fact_esc + can2_margin;
    lote_y_px[i] = can2.width - can2_margin - y_array[i] * fact_esc;
  }
  centroide[0] = centroide[0] * fact_esc + can2_margin;
  centroide[1] = can2.width - can2_margin - centroide[1] * fact_esc;
  ...
  for(let i = 0; (i+1) < len; i++){
    label_px[i] = posicionCentroTexto(lote_x_px[i],lote_y_px[i],lote_x_px[i+1],lote_y_px[i+1], centroide);
    lx_px[i] = label_px[i][0];
    ly_px[i] = label_px[i][1];
  }
  // ==================== saving angles for rotate text ====================
  let misAngulos = [];
  for(let i = 0; (i+1) < len; i++){
    let v_x = lote_x_px[i+1]-lote_x_px[i];
    let v_y = lote_y_px[i+1]-lote_y_px[i];
    misAngulos[i] = calcAngulo(v_x, -v_y);
  }
  // ==================== rendering ====================
  let data = {x_px : lote_x_px,
              y_px : lote_y_px,
              lx_px: lx_px,
              ly_px: ly_px,
              angulos: misAngulos,
